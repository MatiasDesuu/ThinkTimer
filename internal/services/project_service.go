...
func (s *ProjectService) CreateProject(req models.CreateProjectRequest) (*models.Project, error) {
	query := `
		INSERT INTO projects (name, description, url, deadline, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?)
		RETURNING id, name, description, url, deadline, status, created_at, updated_at
	`

	now := time.Now()

	var project models.Project
	err := s.db.QueryRow(query, req.Name, req.Description, req.URL, req.Deadline, now, now).Scan(
...
func (s *ProjectService) GetAllProjects() ([]models.Project, error) {
	query := `
		SELECT id, name, description, url, deadline, status, created_at, updated_at
		FROM projects
